...
const OrganizersEdit = ({ organizer }) => {
    const [formData, setFormData] = useState({
        ...
        direction: organizer.direction || '',
        city: organizer.city || '',
        active: organizer.active || false
    });
    const [logo, setLogo] = useState(null);
    const [logoPreview, setLogoPreview] = useState(
        organizer.logo ? `/storage/${organizer.logo}` : null
    );
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await adminOrganizerService.updateOrganizer(organizer.id, dataToSend);
    ...
    return (
        <div className="container mx-auto p-4">
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-2xl font-bold">Editar Organizador</h1>
                <Link
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
                <div className="flex items-center justify-between">
                    <div>
                        <p className="text-sm text-gray-600">ID: #{organizer.id}</p>
                        <p className="text-sm text-gray-600">
                            Creado: {new Date(organizer.created_at).toLocaleDateString()}
                        </p>
                        {organizer.events_count !== undefined && (
                            <p className="text-sm text-gray-600">
                                Eventos asociados: {organizer.events_count}
                            </p>
                        )}
                    </div>
                    {organizer.events_count > 0 && (
                        <div className="bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-2 rounded">
                            <p className="text-sm">
                                ⚠️ Este organizador tiene eventos asociados
                            </p>
                        </div>
                    )}
                </div>
            ...
            <div className="bg-white shadow-md rounded-lg p-6">
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="mt-6 flex justify-end space-x-4">
                        ...
                        >
                            {loading ? 'Guardando...' : 'Actualizar Organizador'}
                        </button>
                    </div>
                </form>
            </div>
...
OrganizersEdit.layout = page => <AdminLayout>{page}</AdminLayout>;
